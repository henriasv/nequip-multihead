"""Integration test: train -> compile -> inference for multi-head Allegro models.

Tests the full lifecycle:
  1. Train a 2-head Allegro model (Cu + Al) with EMT data
  2. Extract each head and the summed model via modify()
  3. Run single-point inference on a test structure
  4. Verify:
...
Usage:
    python test_allegro_multihead.py [--work-dir DIR]
"""
...
def verify_results(work_dir: Path):
    """Verify extracted heads produce correct outputs."""
    import torch
    ...
    # ---- Test 6: Forces are finite and non-zero ----
    print("\n--- Test 6: Forces are finite and non-zero ---")
    for name, out in [("Cu_head", out_cu), ("Al_head", out_al), ("Summed", out_sum)]:
        forces = out[AtomicDataDict.FORCE_KEY].detach().cpu().numpy()
        assert np.all(np.isfinite(forces)), f"{name} forces contain non-finite values"
        assert np.max(np.abs(forces)) > 1e-8, f"{name} forces are all zero"
        print(f"  {name}: max |F| = {np.max(np.abs(forces)):.6f}")
    print("  PASS")
...
def main():
    ...
    if args.work_dir is None:
        work_dir = Path(tempfile.mkdtemp(prefix="test_allegro_multihead_"))
    else:
    ...
    # ---- Step 1: Train ----
    print("\n" + "=" * 60)
    print("STEP 1: Train multi-head Allegro model")
    print("=" * 60)
    run(
        f"nequip-train"
        f" --config-dir={config_dir}"
        f" --config-name=test_allegro_multihead"
    ...
    # ---- Step 2: Verify ----
    print("\n" + "=" * 60)
    print("STEP 2: Verify head extraction and summed heads")
    print("=" * 60)
